...
import (
	...
	repository "github.com/ruang-guru/playground/backend/golang-http-server/assignment/cashier-app/repository"
)
	...
	Describe("Pay", func() {
		When("pay the total price ", func() {
			It("will return the money changes", func() {
				...
				cartItemRepo.Add(product[1])
				cartItemRepo.Add(product[1])
				Expect(transactionRepo.Pay([]repository.CartItem{}, 50000)).To(Equal(41000))
			})
		})
	})
